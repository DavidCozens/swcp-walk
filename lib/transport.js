...
import { distanceKm, crossesWater } from "./nearby.js";
...
function access(route, locations, from, walkKm) {
  ...
  for (const slug of route.serves || []) {
    ...
    if (!stop) continue;
    const walked = walkKm(from, stop, route);
    if (!Number.isFinite(walked)) continue;
    const km = Math.round(walked * 100) / 100;
    if (!best || km < best.km) best = { stop, km };
  }
  return best;
}

export function routesForSection({ start, end, nearby, transport, locations, walk, crossing }) {
  ...
  // is straight-line, since you'd leave the path to get there anyway — unless
  // that line crosses an estuary, when there's no walk to it at all. The two
  // ends of the day are the two ends of the path. A crossing's "path" is the
  // ferry, which nobody walks: measured along it, every bus to one landing
  // looked a 900 m walk from the other side of the Tamar. So on a crossing only
  // a ferry is taken along it, as far as a landing on the way (St Mawes, between
  // the two Fal ferries); everything else is straight-line, and nothing across
  // the water is in reach.
  ...
  const walkKm = (from, stop, route) => {
    const straight = crossesWater(from.lat, from.lon, stop.lat, stop.lon)
      ? Infinity
      : distanceKm(from.lat, from.lon, stop.lat, stop.lon);
    if (!lengthKm || stop.slug === from.slug) return straight;
    if (crossing && route.type !== "ferry") return straight;
    const fromStart = from === start;
    if (stop.slug === (fromStart ? end : start)?.slug) return lengthKm;
    const m = measured.get(stop.slug);
    if (!m || m.toRoute > (crossing ? 0.2 : onRouteKm)) return straight;
  ...
  return routes
    .map((route) => {
      ...
      return {
        ...
        // A ferry crossing has nowhere to bail out.
        servesEscape: !crossing && (route.serves || []).some((slug) => escapeSlugs.has(slug)),
      };
    })
