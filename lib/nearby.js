...
// Which water the straight line between two points crosses (see
// lib/water.js), by slug. Over a few kilometres, lat/lon is flat enough.
export function waterCrossed(aLat, aLon, bLat, bLon) {
  const p = [aLat, aLon];
  const q = [bLat, bLon];
...
// Whether that line crosses water you can't walk over.
export function crossesWater(aLat, aLon, bLat, bLon) {
  return waterCrossed(aLat, aLon, bLat, bLon).length > 0;
}
